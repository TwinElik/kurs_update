...
def product_description_plain_text(value):
    raw = str(value or "")
    if not raw.strip():
        return ""

    def anchor_to_text(match):
        href = html.unescape(match.group(1) or "").strip()
        label = re.sub(r"<[^>]+>", " ", match.group(2) or "")
        label = " ".join(html.unescape(label).replace("\xa0", " ").split())
    ...
    raw = re.sub(r"<li\b[^>]*>", "• ", raw, flags=re.IGNORECASE)
    raw = re.sub(
        r"</(?:p|div|li|ul|ol|h[1-6]|section|article|table|tr)\s*>",
        "\n",
        raw,
        flags=re.IGNORECASE,
    )
...
def product_description_chunks(product, max_chars=3700):
    text = product_description_plain_text(product.get("description") or "")
    if not text:
        return []
...
async def send_expandable_product_description(message: Message, product):
    for chunk in product_description_chunks(product):
...
async def send_product(callback: CallbackQuery, product_id: int, in_cart=False, back_to_cart=False):
    ...
    if public_media_type == "photo":
        try:
            ...
            await send_expandable_product_description(callback.message, product)
            await callback.message.delete()
        except Exception:
...
async def send_product_card(message: Message, product_id: int, user_id=None):
    ...
    if public_media_type == "photo":
        try:
            ...
            await send_expandable_product_description(message, product)
            return
        except Exception:
            pass
