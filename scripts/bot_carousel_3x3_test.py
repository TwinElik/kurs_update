...
def format_sell_price_for_image(price, lang="ru"):
    price = re.sub(r"\s*сум(?:\s*/\s*гр)?\s*", "", str(price or ""), flags=re.IGNORECASE).strip()
...
def find_latest_generated_diamant_price_image():
    bot_dir = Path(__file__).resolve().parent
    candidates = []
    if KURS_GENERATED_IMAGES_DIR:
        candidates.append(Path(KURS_GENERATED_IMAGES_DIR).expanduser())
    candidates.extend(
        (
            bot_dir.parent / "kurs" / "cache" / "generated",
            bot_dir.parent / "kurs_update" / "cache" / "generated",
            bot_dir.parent / "gold_price_photo_bot" / "cache" / "generated",
        )
    )
    ...
    for directory in candidates:
        ...
        if directory_key in visited or not directory.is_dir():
            continue
        visited.add(directory_key)
        try:
            images.extend(
                path
                for path in directory.rglob("diamant_gold_prices_*.png")
                if path.is_file() and path.stat().st_size > 0
            )
        except OSError as exc:
            logger.warning("Cannot scan generated Diamant price images in %s: %s", directory, exc)
    ...
    try:
        ...
        logger.warning("Cannot select latest generated Diamant price image: %s", exc)
        return None
...
async def send_sell_prices_image(message: Message):
    ...
    texts = SELL_PRICE_IMAGE_TEXTS.get(lang, SELL_PRICE_IMAGE_TEXTS["ru"])
    generated_image = find_latest_generated_diamant_price_image()
    if generated_image:
        photo = FSInputFile(generated_image, filename=generated_image.name)
        logger.info("Using generated Diamant price image: %s", generated_image)
    else:
        ...
        logger.warning("Generated Diamant price image was not found; using legacy renderer")
...
def product_carousel_keyboard(product_id, in_cart=False, back_to_cart=False, user_id=None):
    """Keyboard for Telegram rich slideshow cards: no manual media arrows."""
    rows = []
    site_button = InlineKeyboardButton(
        text=tr(user_id, "site"),
        url=product_site_link(product_id).replace("&amp;", "&"),
    )
    if in_cart:
        rows.append([site_button])
        rows.append(
            [
                InlineKeyboardButton(text=tr(user_id, "in_cart"), callback_data="cart_view"),
                InlineKeyboardButton(
                    text=tr(user_id, "remove_selection"),
        ...
        if back_to_cart:
            rows.append(
                [InlineKeyboardButton(text=tr(user_id, "back_to_cart"), callback_data="cart_view")]
            )
    else:
        rows.append([site_button])
        rows.append(
            [InlineKeyboardButton(text=tr(user_id, "add_to_cart"), callback_data=f"cart_add:{product_id}")]
        )
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def cart_text(items, user_id=None):
    ...
    if user_lang(user_id) == "uz":
        ...
        if delivery_price > 0:
            ...
            lines.append("🚚 4 g dan boshlab yetkazib berish bepul. 4 g dan kam buyurtma uchun — 30 000 so'm.")
        else:
        ...
        if delivery_price > 0:
            ...
            lines.append("🚚 При заказе от 4 г доставка бесплатная. Если общий вес меньше 4 г — доставка 30 000 сум.")
        else:
...
def public_media_urls_for_product(product_id):
    ...
    return [first_photo, primary] + unique_extras
...
async def send_rich_product_slideshow(message: Message, media_items, caption, reply_markup):
    """
    Send a Bot API Rich Message slideshow directly over HTTP.

    media_items: [(url, "photo"|"video"), ...]
    Returns True if Telegram accepted the rich message.
    """
    slideshow_items = []
    for url, media_type in (media_items or [])[:3]:
        safe_url = html.escape(str(url), quote=True)
        if media_type == "video":
            slideshow_items.append(f'<video src="{safe_url}"/>')
        else:
            slideshow_items.append(f'<img src="{safe_url}"/>')

    if not slideshow_items:
        return False

    # Keep product text inside the slideshow caption, so the inline keyboard
    # belongs to the same rich message.
    rich_html = (
        "<tg-slideshow>"
        + "".join(slideshow_items)
    ...
    try:
        ...
        async with session.post(api_url, json=payload) as response:
            ...
            if response.status == 200 and result.get("ok"):
                return True
            logger.warning(
                "sendRichMessage slideshow failed: status=%s response=%s",
                response.status,
                result,
            )
    except Exception:
        logger.exception("sendRichMessage slideshow failed")
    return False
...
async def send_product_card(message: Message, product_id: int, user_id=None):
    ...
    media_items = public_media_urls_for_product(product_id)[:3]
    ...
    inline_markup = product_carousel_keyboard(product_id, user_id=user_id)

    # Preferred path: Telegram Rich Message slideshow, max 3 media.
    if media_items:
        if await send_rich_product_slideshow(
            message,
            media_items,
            caption,
            inline_markup,
    ...
    # Safe fallback for older/unsupported Bot API/client behavior:
    # send only the first media exactly like before.
    if public_media_type == "photo":
        try:
            data, filename, content_type = await download_public_media(public_media)
            if not content_type.startswith("image/"):
...
async def prepare_product_card(product_id: int, user_id=None):
    """Prepare a catalog card. Rich slideshow uses public URLs directly."""
    product = get_product(product_id)
    if not product:
        return None
...
async def send_prepared_product_card(message: Message, prepared):
    """Send one prepared card in order; use up to 3 media in Telegram slideshow."""
    if not prepared:
        return

    caption = prepared["caption"]
    markup = prepared["reply_markup"]
    media_items = (prepared.get("media_items") or [])[:3]

    if media_items:
        if await send_rich_product_slideshow(message, media_items, caption, markup):
            await send_product_description_messages(
                message,
                prepared.get("description_messages"),
            )
            return

        # Fallback: if rich slideshow is rejected, only download/send first media.
        public_media, public_media_type = media_items[0]
        try:
            data, filename, content_type = await download_public_media(public_media)
            valid = (
                public_media_type == "photo" and content_type.startswith("image/")
            ) or (
                public_media_type == "video" and content_type.startswith("video/")
            )
            if valid:
                if public_media_type == "photo":
                    await message.answer_photo(
            ...
            logger.exception("Fallback media send failed for product %s", prepared.get("product_id"))
...
async def prepare_product_cards(products, user_id=None):
    """Download media for all cards concurrently while preserving input order."""
    tasks = [
...
async def send_cards_page(callback: CallbackQuery, products, offset, kind, category_id=None):
    ...
    await callback.message.answer(
        f"Показал {len(page_products)} товара. Можно открыть следующий набор.",
...
async def article_search_query_message(message: Message, bot: Bot):
    ...
    products = find_products_by_model(query, limit=5)
    if not products:
        await message.answer(
...
async def category_products_cards(callback: CallbackQuery):
    ...
    if len(products) == 5:
        products = products + get_products_for_category(category_id, offset + PAGE_SIZE)[:1]
