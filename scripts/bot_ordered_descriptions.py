...
async def send_cards_page(callback: CallbackQuery, products, offset, kind, category_id=None):
    ...
    # Отправляем карточки строго последовательно, чтобы описание каждого товара
    # всегда шло сразу после своей карточки.
    for product in page_products:
        await send_product_card(callback.message, product["product_id"], callback.from_user.id)
...
async def send_search_results(message: Message, user_id, category_key, offset=0):
    ...
    async def send_current_page():
        for product in products[:PAGE_SIZE]:
            ...
            if SEARCH_TOKENS.get(user_id) != search_token or current_state.get("view") != "search_results":
                return
            await send_product_card(message, product["product_id"], user_id)
