...
MANUAL_SYNC_LIMIT = int(os.getenv("MANUAL_SYNC_LIMIT", "5"))
MANUAL_SYNC_TIMEOUT_SECONDS = int(os.getenv("MANUAL_SYNC_TIMEOUT_SECONDS", "15"))
DB_CONFIG = {
...
async def run_manual_sync(force_latest=False):
    ...
    def run_in_thread():
        if latest_generation:
            ...
            results = asyncio.run(enqueue_and_send_site_sync_jobs(event, force=False))
            if results:
                return results
